...
export async function getVpnStatus(
    ...
    const response = await typedFetch<ServerStatusResponse>('http://localhost:8080/vpn', {
        method: 'GET',
        headers
    });
...
export async function vpnOperation(operation: string) {
    ...
    headers.append('Authorization', `Bearer ${token}`);
    const response = await typedFetch<ServerStatusResponse>('http://localhost:8080/vpn/operation', {
        method: 'POST',
        headers,
