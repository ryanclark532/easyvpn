...
func CreateUser(Username string, Name string, Password string, IsAdmin bool) (dtos.User, error) {
	db, err := GetDB()
	if err != nil {
		return dtos.User{}, err
	}
	query := "INSERT INTO Users (username, name, password, IsAdmin) VALUES (?, ?, ?, ?)"
	_, err = db.Exec(query, Username, Name, Password, IsAdmin)
	if err != nil {
		return dtos.User{}, err
	}
	return dtos.User{
