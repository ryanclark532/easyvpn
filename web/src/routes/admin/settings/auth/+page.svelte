...
<div class="flex">
	...
	<div class="w-full p-5">
		<div class="mb-4">
			...
			<P
				>Authentication Settings control how VPN clients authenticate with the server.</P
			>
		</div>
<Heading tag="h4" class="mb-1">Maximum Authentication Attempts</Heading>
		...
		<Tooltip
			>After the maximum amount of attempts a users account will be locked</Tooltip
		><Heading tag="h4" class="mb-1">Lockout Timeout</Heading>
		<Input placeholder="500000..." class="mb-4" bind:value={data.lockout_timeout}/>
		<Tooltip
			>The amount of time a user will be locked out after incorrect password attempts. Expressed in miliseconds</Tooltip
		>
		...
		<Tooltip>Whether VPN users will be forced to set strong passwords. A strong password must be 8 characters and contain at least one capitol, number and special symbol</Tooltip>
		<Button
			class="w-full"
			on:click={() => {
				open = !open;
